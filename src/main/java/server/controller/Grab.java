...
import server.model.WeaponCard;
 ...
 public class Grab {
    ...
    public static boolean grabWeaponCard(Player grabber,int numWeapon){

        //1. Choose 1 of the spawnpoint's 3 weapons. And
        //Take Weapon and deleted from Map
        WeaponCard gotWeaponCard;
        gotWeaponCard=grabber.getLobby().getMap().getSquare(grabber.getPosition()).removeWeaponCardFromDeck(numWeapon);
        if (gotWeaponCard==null)
            return false;

        if (payForWeapon(grabber,gotWeaponCard)) {
    ...
    public static boolean grabWeaponCard(Player grabber,int numWeapon,int numWeaponSwitch){
        ...
        gotWeapon=grabber.getLobby().getMap().getSquare(grabber.getPosition()).removeWeaponCardFromDeck(numWeapon);
        if (gotWeapon==null)
            return false;
        exchangeWeapon=grabber.getWeaponCard().remove(numWeapon);
        grabber.getLobby().getMap().getSquare(grabber.getPosition()).getWeaponCardDeck().add(exchangeWeapon);

        if (payForWeapon(grabber,gotWeapon)) {
            grabber.addWeaponCard(gotWeapon);
            return true;
        }
        else
            return false;
    ...
    private static boolean payForWeapon(Player grabber, WeaponCard gotWeaponCard){
